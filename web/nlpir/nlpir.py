...
def Seg_list(paragraph):

    split_list=[]
    for t in Seg(paragraph):
        split_list.append(t[0])
    return split_list


def Seg_purelist(paragraph):
    split_list = []
    pure_split_list=[]
    flag_w = False
    split_word = Seg(paragraph)
    num_sentence = 0
    tx = ""



    for e,t in enumerate(split_word):
        split_list.append(t[0])
        tx=t[1]
        if tx[0]=="w" :
            if (tx == "wj" or tx =="ww" or tx =="wt" or tx =="ws" or tx =="wp") and flag_w == False:
                num_sentence +=1
                pure_split_list.append('。')
                flag_w =True
        else:
            pure_split_list.append(t[0])
            flag_w = False

    if tx != "wj" and tx !="ww" and tx !="wt" and tx !="ws" and tx !="wp":
        pure_split_list.append('。')
        num_sentence +=1

    return split_list,pure_split_list,num_sentence

# UserWord = []

def mImportUserDict(filename):
    global UserWord
...
if __name__ == "__main__":
    ...
    for t in Seg(p):
        s = '%s\t%s\t%s' % (t[0].encode('gbk'),t[1],translatePOS(t[1]).encode('gbk'))
        print(s)
